...
def parse_biotype(x):
    ...
    if x.shape[1] != 2:
        sys.exit(
            "ERROR: Biotype file should have two columns of " "gene names and biotype"
        )
...
def parse_cor(x):
    ...
    if x.shape[1] != 2:
        sys.exit(
            "ERROR: Correlation file should have two columns "
...
def cont_method(x, y):
    ...
    xx["group"] = pd.cut(
        xx[y],
        bins=[np.min(xx[y]), np.median(xx[y]), np.max(xx[y])],
        labels=["Low", "High"],
        include_lowest=True,
    )
...
def get_ascni(prior_m, expression_m, mad, p, s, a, nn=".csv"):
    ...
    for i in prior_m:
        ...
        res = main_model_prediction(
            inp=expression_m, d_list=prior_m[i], gi=i, lp=in_p, s=s, run=run_info
        )
...
def get_cor(
    ...
    if mtb <= 0:
        sys.exit(
            "Gene did not pass the MAD threshold. Try setting a "
    ...
    if docor:
        ...
        if len(gcor.index) < 19:
            sys.exit(
                "You don't have enough genes that show correlation "
...
def make_prior1(npc, gcor):
    ...
    for i in range(6, npc + 1):
        cc.append(str(i))
    df = pd.DataFrame(
        np.random.randint(1, 2, size=(len(gcor.index), int(len(cc)))), columns=cc
    )
    sd = 1
    ed = 3
    for i in cc:
...
def make_prior2(hc, gt):
    ...
    cc2 = ["V1", "V2"]
    prior2 = pd.DataFrame(
        np.random.randint(1, 2, size=(len(hc.index), int(len(cc2)))), columns=cc2
    )
...
def save_merged_n_d_ac_fd_co(n, d, ac, fd, co, path, nfile, run_info):
    ...
    dbs_results = dbs.ACSNIDeriveResults(
        ac=ac, n=n, d=d, fd=fd, co=co, run_info=run_info
    )
