...
def check_input(nc):
    ...
    if nc <= 15:
        sys.exit("{} samples is small for the tool".format(nc))
    elif nc >= 800:
        sys.exit("{} samples is large, use " "ACSNI-split".format(nc))
    else:
...
def check_name(e_df):
    ...
    if n_col >= 2:
        ...
        sys.exit(
            "Remove the {} extra label columns in "
...
def main_model_prediction(inp, d_list, gi, lp, s, run):
    ...
    if run.get("f") == 1:
        ...
        net.get_predicted(run.get("f")).to_csv(
            "P{}_{}_{}".format(str(s), gi, run.get("i")), index=False
        )
